...
position 属性为 static 、 relative 或 sticky：最近的祖先块元素（inline-block, block 或 list-item）的**内容区**的边缘组成

position 属性为 absolute：最近的 position 的值不是 static 的祖先元素的**内边距区**的边缘
